...
# Menu de navegação
pagina = st.sidebar.radio("📂 Menu", ["📊 Cálculo do Consumo", "📘 Sobre o Modelo Estatístico"])

if pagina == "📊 Cálculo do Consumo":
    ...
    if aba == "🧮 Cálculo do Consumo":
        ...
        if os.path.exists("docs"):
            ...
            if paginas:
                ...
                st.warning("A pasta 'docs' não contém imagens PNG.")
        else:
            st.warning("Pasta 'docs' não encontrada. Verifique se as imagens foram enviadas corretamente.")
elif pagina == "📘 Sobre o Modelo Estatístico":
    st.markdown("## 📘 Sobre o Modelo Estatístico")
    st.markdown("Visualize abaixo o conteúdo técnico referente ao modelo estatístico utilizado.")
    ...
    with col3:
        if st.session_state.pagina_atual < len(paginas) - 1:
            if st.button("Próxima página ➡️"):
                st.session_state.pagina_atual += 1
    st.image(f"docs/{paginas[st.session_state.pagina_atual]}", use_container_width=True)
