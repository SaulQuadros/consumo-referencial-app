...
# Persistência do DataFrame no session_state
...
# Para uma população de 50.000 habitantes os valores originais podem estar, por exemplo, em torno de 300.000 m³/mês.
# Para 80.000 habitantes, usaremos um fator de 1.6, resultando em uma média de aproximadamente 480.000 m³/mês.
# Usaremos uma distribuição normal com média 480.000 e desvio padrão de 50.000.
consumo_valores = np.random.normal(480000, 50000, 12).astype(int)

# Cria o DataFrame
df_novo = pd.DataFrame({
    "Mês": meses,
    "Consumo (m³)": consumo_valores
})

# Salva o DataFrame em um arquivo CSV
df_novo.to_csv("Cons-Hist-M1_2025_80k.csv", index=False)
print("Arquivo 'Cons-Hist-M1_2025_80k.csv' gerado com sucesso!")
...
# Persistência do DataFrame no session_state para "Cálculo do Consumo"
if "df_consumo" not in st.session_state:
    st.session_state.df_consumo = None

# Menu de navegação com três opções
aba = st.sidebar.radio("Navegar para:", ["🧮 Cálculo do Consumo", "📘 Sobre o Modelo Estatístico", "📊 Gerar Histograma Consumo"])
...
if aba == "🧮 Cálculo do Consumo":
    ...
    st.title("📘 Sobre o Modelo Estatístico")
    # Mantemos apenas o botão de download do PDF
    pdf_file = "03_Estatistica_2025.pdf"
    if os.path.exists(pdf_file):
        with open(pdf_file, "rb") as f:
            pdf_bytes = f.read()
    ...
    if st.button("Criar Planilha"):
        if ano_final < ano_inicial:
            ...
            meses = ["Janeiro", "Fevereiro", "Março", "Abril", "Maio", "Junho",
                     "Julho", "Agosto", "Setembro", "Outubro", "Novembro", "Dezembro"]
            # Base para 50.000 habitantes: média de 300.000 m³/mês e desvio de 50.000
            fator = populacao / 50000
            ...
            # Permite visualizar a tabela gerada
            st.dataframe(df_gerado)
            
            # Converte para CSV e oferece para download
